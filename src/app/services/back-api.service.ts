...
export class BackApiService {
  ...
  private getHeaders(): HttpHeaders {
    ...
    try {
      ...
      if (authToken) {
        ...
        console.error(
          "Le jeton d'authentification est introuvable dans le localStorage."
        );
      }
    } catch (error) {
      // Gérer les erreurs lors de la récupération du jeton d'authentification
      console.error(
        "Une erreur s'est produite lors de la récupération du jeton d'authentification :",
        error
      );
    }
    return headers;
  ...
  postCreate(postData: FormData): Observable<PostModel[]> {
    ...
    return this.http.post<PostModel[]>(url, postData, {
